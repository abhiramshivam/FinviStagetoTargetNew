...
SELECT 
	...
	CASE WHEN TRN.AFTRTYP = 'NSFDBJ' THEN  TRN.TRN_AMT * -1 ELSE TRN.TRN_AMT END AS TXN_AMT ,
	0 AS CREDIT,
	...
	ADJ.ADJUSTMENT_ID AS ADJUSTMENT_ID,
	ACCOUNTID,	
	SOURCETRANSACTIONID,
...
( 
	...
	WHERE AFTRTYP IN (SELECT code FROM COM_FINVI_OASIS_TRANSACTIONCODES_{PP_TenantName}.AJ_CORE_TRANSACTION_CODE TC WHERE  TC.TRANSACTION_TYPE ='Adjustment')  
) TRN
...
LEFT JOIN COM_FINVI_OASIS_MACRO_ACCOUNT_{PP_TenantName}.T_DATA_ADJUSTMENT ADJ 
	ON ADJ.MIG_SRC_TRAN_ID = TRN.SOURCETRANSACTIONID AND ADJ.MIG_SRC_ACCT_ID = TRN.ACCOUNTID 